...
import GET_GIVE_FEED_FEATURES from './getGiveFeedFeatures';
...
class FeaturesFeedConnected extends PureComponent {
  ...
  render() {
    ...
    return (
      <Query query={GET_GIVE_FEED_FEATURES} fetchPolicy="cache-and-network">
        {({ error, data, loading, refetch }) => {
          const features = get(data, 'giveFeedFeatures', []);
          this.refetchRef({ refetch, id: 'giveFeed' });
          return (
            <FeedView
              error={error}
              content={features}
